...
class PaperUnitTest
{
    ...
    function __construct()
    {
        ...
        $this->csvFilePath = 'relis_app/helpers/tests/testFiles/paper/csvPapers.xls';
    ...
    function run_tests()
    {
        ...
        $this->importEndNote();
        $this->importBibtext();
        $this->importPapersLoadCSV();
        $this->importPapersSaveCSV();
        $this->importPapersLoadBibtext();
        $this->importPapersSaveBibtext();
     ...
     * Description : Test the loading and processing of CSV files for importing papers
     */
    private function importPapersLoadCSV()
    {
        $action = "import_papers_load_csv";
        $test_name = "Test the loading and processing of importing papers from CSV files";
     ...
     * Description : inserting the loaded csv papers into the database
     * Expected papers inserted in DB : 5
     */
    private function importPapersSaveCSV()
    {
        $action = "import_papers_save_csv";
        $test_name = "Inserting the loaded csv papers into the database";
     ...
     * Description : Test the loading and processing of bibtext files
     */
    private function importPapersLoadBibtext()
    {
        $action = "import_papers_load_bibtext";
        $test_name = "Test the loading and processing of bibtext files";
        $test_aspect_httpCode = "Http response code";
        $expected_httpCode = http_code()[200];
     ...
     * Expected papers inserted in DB : 5
     */
    private function importPapersSaveBibtext()
    {
        $action = "import_papers_save_bibtext";
        $test_name = "Inserting the loaded bibtext papers into the database";
        $test_aspect_papersInDB = "Nbr of papers in project DB";
        $expected_nbrOfPapersInDB = (string) ($this->ci->db->query("SELECT COUNT(*) AS row_count FROM relis_dev_correct_" . getProjectShortName() . ".paper")->row_array()['row_count'] + 5);
